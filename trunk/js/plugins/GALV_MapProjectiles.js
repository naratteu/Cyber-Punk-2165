...
Galv.PROJ.getTarget = function(id) {
	...
	} else if (id.hasOwnProperty('x') && id.hasOwnProperty('y')) {
		if (id.hasOwnProperty('_characterName') == false) {
			id._characterName = true;
		}
		return id;
	} else if (Array.isArray(id)) {
		// id was an array [x,y]
		return {x: id[0], y: id[1], _characterName: true};
	} else {
		console.log("Galv.PROJ.getTarget : 처리할수없는 데이터구조 입니다.");
		return id;
	}
};
